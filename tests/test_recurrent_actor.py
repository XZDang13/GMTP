...
from model.actor import RecurrentActor
...
def test_recurrent_actor_sequence_normalizer_flattens_time_and_batch():
    ...
    step, next_state = actor(obs, initial_state=actor.get_initial_state(2), episode_starts=episode_starts, update_normlizer=True)
...
def test_policy_rnn_state_roundtrip_through_sequence_batches():
    ...
    for step_idx in range(3):
        buffer.add_records(
            {
                "policy_observations": torch.full((2, 4), float(step_idx)),
                "episode_starts": torch.tensor([step_idx == 0, step_idx == 0]),
                "policy_rnn_state": torch.stack(
                    (
                        torch.full((2, 8), 10.0 * step_idx),
                        torch.full((2, 8), 10.0 * step_idx + 1.0),
                    ),
                    dim=0,
                ),
            }
    ...
    batch = next(
        buffer.sample_sequence_batches(
            key_names=["policy_observations", "episode_starts"],
    ...
    assert batch["policy_rnn_state_init"].shape == (2, 2, 8)
    torch.testing.assert_close(batch["policy_rnn_state_init"][0], torch.zeros(2, 8))
    torch.testing.assert_close(batch["policy_rnn_state_init"][1], torch.ones(2, 8))
...
def test_recurrent_policy_loss_accepts_sequence_batches_from_rollout_buffer():
        ...
        buffer.add_records(
            {
                ...
                "episode_starts": episode_starts,
                "policy_rnn_state": policy_state.transpose(0, 1),
            }
        )
    ...
    loss_dict = PPO.compute_policy_loss_recurrent(
        ...
        initial_state=batch["policy_rnn_state_init"].transpose(0, 1).contiguous(),
...
def test_eval_and_sim2sim_rebuild_recurrent_actor_from_kwargs():
    actor = RecurrentActor(obs_dim=5, action_dim=2, hidden_size=8, num_layers=2)
    actor_weights = actor.state_dict()
    actor_kwargs = {"hidden_size": 8, "num_layers": 2}

    eval_actor = Evaluator._build_actor({"policy": 5}, "recurrent", 2, 3, actor_kwargs=actor_kwargs)
    sim_actor = Sim2SimEvaluator._build_actor({"policy": 5}, "recurrent", 2, 3, actor_kwargs=actor_kwargs)

    eval_actor.load_state_dict(actor_weights)
    sim_actor.load_state_dict(actor_weights)

    assert isinstance(eval_actor, RecurrentActor)
    assert isinstance(sim_actor, RecurrentActor)


def test_recurrent_actor_kwargs_are_inferred_from_gru_weights():
    actor = RecurrentActor(obs_dim=5, action_dim=2, hidden_size=8, num_layers=2)
    actor_weights = actor.state_dict()
    expected = {"hidden_size": 8, "num_layers": 2}

    assert Evaluator._infer_recurrent_actor_kwargs(actor_weights) == expected
    assert Sim2SimEvaluator._infer_recurrent_actor_kwargs(actor_weights) == expected
